...
int graphic_interface(tree* data) 
{
    ...
    {        
        ...
        {
            static node_t* node = data->root
            
            while (node->left != NULL && node->right != NULL)
            {
                selection = akinator(node);
                if (selection == 1) node = node->left
                else if (selection == 1) node = node->left
                else if (selection == 3) currentScreen = 0;
            }
        }
...
int akinator(node_t* node)
{
    ...
    draw_centered_text(node->val, 300, 36, TX_BLACK);

    if (is_button_clicked(btn1) && (node->left == NULL & node->right == NULL)) 
    {
        draw_centered_text("dsadasd", 300, 36, TX_BLACK);
    }
